...
class LanguageHelper extends MyHelper {
	...
	public function setup($filename, $availLangs=array()) {
		...
		public function setAvailLangs($languages) {
			...
			if($new == $_SESSION["AvailLangs"]) {
				return;
			}

			$this->logger->debug("setAvailLangs: new={print_r($new)}");
	...
	//
		...
		private function getLanguage($language) {
			if(!in_array($language, $_SESSION["AvailLangs"])) {
				return NULL;
			}
	...
	//
		...
		public function languageCookieGet() {
			...
			$theCookieHelper->bake($this->session, $_SESSION[$this->session]);
		}
	//
		/**
		 * internal dictionary.
		 ...
		 */
		public function translate($word, $bFemale=false) {
			global $theTranslators;
			$back = $theTranslators->getWord($_SESSION["language"], $word, $bFemale);
			$this->logger->debug("translate($word, " . (int)$bFemale . ") --[{$_SESSION['language']}]-> $back");
			return $back;
		}
	//
		/**
		...
		public function changeSessionLang($newlang="") {
			...
			if(isset($_GET["language"]) && $_GET["language"] != "") {
				$this->logger->trace("changeSessionLang GET={$_GET['language']}");

				if(in_array($_GET["language"], $_SESSION["AvailLangs"])) {
			...
			if($newlang == ""
					&& isset($_COOKIE[$this->session])
					&& $_COOKIE[$this->session] != ""
					&& in_array($_COOKIE[$this->session], $_SESSION["AvailLangs"])) {
				$newlang = $_COOKIE[$this->session];
	...
	//
		...
		public function languages() {
			...
			if(!in_array($_SESSION["language"], $which)) {
				$_SESSION["language"] = $which[0];
			}

			// Load the required languages
			foreach($which as $lang) {
				...
				$back .= $theBodyHelper->anchor(
					"{$this->filename}?language=$lang",
					"<img class=\"limg\" title=\"$lang\" alt=\"$lang\" src=\"$pic\" />"
				);
			}
